...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
	...
	float h = 2.0f * tanf(vertical_fov / 180.0f * PI_F / 2.0f);
	float w = aspect_ratio * h;

	float x = sensor_pixel.x / float(film.width) * w - w/2;
	float y = sensor_pixel.y / float(film.height) * h - h/2;

	// float scale = tanf(vertical_fov / 180.0f * PI_F * 0.5f);
	// float x = (sensor_pixel.x - float(film.width) / 2.0f) * 2.0f / float(film.width) * scale * aspect_ratio;
	// float y = (sensor_pixel.y - float(film.height) / 2.0f) * 2.0f / float(film.height) * scale;
	//Build ray:
	Ray ray;
	ray.point = Vec3(); //ray should start at the origin
	ray.dir = Vec3(x, y,-1.0f).unit(); //TODO: compute from sensor plane position
	ray.depth = film.max_ray_depth; //rays should, by default, go as deep as the max depth parameter allows
	ray.throughput = Spectrum(1.0f);
