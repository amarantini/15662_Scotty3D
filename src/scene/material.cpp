...
Vec3 reflect(Vec3 dir) {
	...
	// with normal (0,1,0)
	Vec3 w_r = - dir + 2 * dir.y * Vec3(0,1,0);
...
Vec3 refract(Vec3 out_dir, float index_of_refraction, bool& was_internal) {
	...
	float cosO = - out_dir.y;
	float etaI = 1.0f, etaT = index_of_refraction;
	Vec3 n(0,1,0);
	if(cosO<0){ // outside surface
		cosO = - cosO;
	} else { // inside surface
		std::swap(etaI, etaT);
		n = -n;
	...
	float eta = (etaI/etaT);
	float cos2I = 1.0f - eta * eta * (1.0f - cosO * cosO);
	if(cos2I<0){
		// total internal reflection
		was_internal = true;
		return Vec3{0,0,0};
	}
	Vec3 dir = (eta * - out_dir + (eta * cosO - sqrtf(cos2I)) * n).unit();
...
float schlick(Vec3 in_dir, float index_of_refraction) {
	...
	R0 *= R0;
	return R0 + (1.0f-R0) * pow((1-cosO),5.0f);
}
...
Spectrum Lambertian::evaluate(Vec3 out, Vec3 in, Vec2 uv) const {
	...
	if(in.y > .0f){
		return (albedo.lock()->evaluate(uv) / PI_F) * dot(in, Vec3(0,1,0));
	} else {
		return {};
	}
...
Scatter Lambertian::scatter(RNG &rng, Vec3 out, Vec2 uv) const {
	...
	ret.direction = sampler.sample(rng).unit();
